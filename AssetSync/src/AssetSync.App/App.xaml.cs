...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // API services (transient so they read fresh config each time)
        ...
        var setupComplete = configRepo.GetAsync(ConfigKeys.SetupComplete).GetAwaiter().GetResult();
        if (setupComplete != "true")
        {
            var wizardVm = Services.GetRequiredService<SetupWizardViewModel>();
            var wizard = new Views.SetupWizard.SetupWizardWindow(wizardVm);
            wizard.ShowDialog();
        }
    }
}
